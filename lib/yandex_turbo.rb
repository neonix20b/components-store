...
class YandexTurbo
        ...
        xml.channel do
          ...
          products.each do |product|
            ...
            xml.item(turbo: 'true') do
              ...
              xml.link Spree::Core::Engine.routes.url_helpers.product_url(product, host: store.url,
                                                                                   protocol: 'https')
              xml['turbo'].send('source',
                                Spree::Core::Engine.routes.url_helpers.product_url(product, host: store.url,
                                                                                            protocol: 'https'))
  ...
  def self.request(url, data: nil)
    ...
    if data.nil?
      ...
      resp = HTTParty.post("https://api.webmaster.yandex.net/v4#{url}",
                           headers: { 'Authorization' => "OAuth #{token}", 'Content-Type' => 'application/rss+xml' }, body: data)
    end
    puts resp
    resp
  end
