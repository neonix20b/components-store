class Spree::MessagesController < Spree::StoreController
  ...
  def email
    return unless current_spree_user.admin?
    ...
    message = current_store.emails.new(from: params[:from], to: params[:email], subject: params[:subject],
                                       body: params[:body], read: true, direction: :out)
  ...
  def router
    if params[:key] == ENV['MAILGUN_KEY'] and params['body-mime'].present?
      ...
      order = Spree::Order.find_by(number: subject[/\b(\w\d{8,})\b/, 1])
      order = Spree::Order.find_by(number: body[/\b(\w\d{8,})\b/, 1]) if order.nil?

      order = Email.where(to: from).or(Email.where(from:)).order(created_at: :desc).first.try(:order) if order.nil?

      message = store.emails.new(from:, to:, subject:, body:, direction: :in)
